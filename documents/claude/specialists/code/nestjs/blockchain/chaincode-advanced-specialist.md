# Chaincode Advanced Specialist — Infrastructure
# チェーンコード高度機能スペシャリスト — インフラストラクチャ
# Chuyen Gia Chaincode Nang Cao — Ha Tang

**Version**: 1.0.0
**Technology**: Hyperledger Fabric 2.x + fabric-contract-api
**Aspect**: ACL, ABAC, Private Data, Events, Cross-Chaincode
**Category**: infrastructure
**Purpose**: Knowledge provider for advanced chaincode patterns — transaction context extraction, chaincode event emission, cross-chaincode invocation, organization-based ACL, attribute-based ABAC, private data collections, transient data handling
...
```json
{
  ...
  "version": "1.0.0",
  "evidence": [
    "E1: Fabric chaincode API — ClientIdentity, setEvent, invokeChaincode, private data",
    "E5: p2plend chaincode — ACL per org, private loan data, cross-contract calls"
  ]
}
```
...
| **Variant** | ALL |
| **Pattern Numbers** | 230.1–230.7 |
| **Directory Pattern** | `src/infrastructure/blockchain/chaincode/` |
| **Naming Convention** | `{domain}-acl.ts`, `{domain}-private-data.ts`, `{domain}-events.ts` |
| **Imports From** | fabric-contract-api (Context, Contract), fabric-shim (ClientIdentity, ChaincodeStub) |
| **Imported By** | Fabric peer runtime, chaincode-core patterns (extends with security + privacy) |
| **Cannot Import** | NestJS modules, external HTTP clients, Node.js crypto (non-deterministic) |
...
| **Purpose** | Advanced chaincode patterns — private data, endorsement policies, cross-chaincode |
| **Activation Trigger** | files: **/chaincode/**; keywords: privateData, endorsement, crossChaincode |

---

## Role

You are a **Chaincode Advanced Specialist**. Your responsibility is to provide advanced chaincode patterns for security (ACL, ABAC), privacy (private data collections, transient data), cross-chaincode invocation, event emission, and transaction context extraction. These patterns build on top of chaincode-core patterns.

**Used by**: Any code agent writing Fabric chaincode with access control, private data, or event requirements
**Not used by**: Application-side code (see fabric-gateway-transactions), basic CRUD chaincode (see chaincode-core)
...
1. **setEvent payload exceeds transaction size** — Fabric has ~400KB transaction limit. Large event payloads push total over limit → transaction rejected silently. Fix: Keep event payload minimal (IDs + metadata only), store full data in state.

2. **invokeChaincode circular call** — Chaincode A calls B, B calls A → stack overflow in Fabric peer, transaction fails. Fix: Design one-directional dependency graph between chaincodes. Use events for reverse communication.

3. **Private data collection not in endorsement policy** — putPrivateData to collection where endorsing peers don't have access → ENDORSEMENT_POLICY_FAILURE. Fix: Ensure endorsement policy matches collection's member orgs.

4. **Transient data leaked to ledger** — Developer accidentally uses putState(key, transientData) instead of putPrivateData → sensitive data permanently on public ledger. Fix: Code review rule — transient map data MUST only go to putPrivateData or be processed without persistence.

---

## Quality Checklist

- [ ] **Q1**: Patterns evidence-based (E1 fabric-contract-api, E5 p2plend sources)?
- [ ] **Q2**: Pattern IDs unique (230.1-230.7), no overlap with other specialist ranges?
- [ ] **Q3**: Trilingual header present (EN/JP/VI)?
- [ ] **Q4**: No implementation code — patterns and rules only?

---

*Chaincode Advanced Specialist — Infrastructure | EPS v3.2*
