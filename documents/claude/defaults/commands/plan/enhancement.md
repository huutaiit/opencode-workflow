# plan/enhancement.md — Step 1C: Enhancement Plan Workflow

## Step 1C: Enhancement Plan Workflow

**For enhancement tasks** (when Step 0.5 detected taskType === 'enhancement').
...
**Note for context-loading**: When invoked from enhancement workflow, all Steps 2.5-2.10 run normally. Step 2.8 sets ddConfig defaults (mode: "none"). Step 2.9 loads full specialists. Step 2.10 loads architecture context from specialist → arch-detect → RAG.
...
**Note for context-loading**: All Steps 2.5-2.10 run normally for enhancement workflows. Step 2.8 sets ddConfig defaults (no DD available). Architecture context loads via specialist → arch-detect → RAG (Step 2.10). This ensures plan steps have full architecture compliance context.
